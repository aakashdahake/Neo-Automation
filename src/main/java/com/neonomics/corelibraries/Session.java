...
public class Session extends Authorization {
	...
	public String getSessionID(String bankID, HashMap<String, String> headData) {
		...
		Response resp = RestAssured.given().contentType(ContentType.JSON).headers(headData).body(payload).when().post(Endpoints.CREATE_SESSION_ID).then().assertThat()
				.statusCode(HttpStatus.SC_CREATED).assertThat().body(JsonSchemaValidator.matchesJsonSchema(SessionIdSchema)).extract().response();
	...
	public HashMap<String, String> getSessionStatus(String sessionID, HashMap<String, String> headData) {
		...
		Response resp = RestAssured.given().accept(ContentType.JSON).pathParam("sessionId", sessionID).headers(headData).get(Endpoints.SESSION_STATUS).then().assertThat()
				.statusCode(HttpStatus.SC_OK).assertThat().body(JsonSchemaValidator.matchesJsonSchema(SessionStatusSchema)).extract().response();
	...
	public void terminateSession(String sessionID, HashMap<String, String> headData) {
		...
		RestAssured.given().accept(ContentType.JSON).headers(headData).pathParam("sessionId", sessionID).delete(Endpoints.SESSION_STATUS).then().assertThat()
