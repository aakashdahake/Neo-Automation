...
public class StepDefinitions_Functional_Test implements ConstantsRef {
	...
	@Then("user retrieves bank ID for {string} provided x-device-id as {string}")
	public void user_retrieves_bank_id_for_provided_x_device_id_as(String bankName, String xDeviceID) {
	...
	@Then("user {string} session for {string} with x-device-id as {string}")
	public void user_session_for_with_x_device_id_as(String action, String bankName, String xDeviceID) {
		...
		try {
			...
			case SESSION_CREATE:
				...
				break;
			case SESSION_DELETE:
				logInstance.info("Deleting session for bank [{}] when Device ID [{}] with token [{}]", bankName,
						xDeviceID);
	...
	public void user_validates_session_status_for(String bankName) {
		try {
			logInstance.info("Getting deatils for [{}] bank", bankName);
	...
	@Then("user checks for {string} consent and {string} if required provided redirect URL as {string} and x-device-id as {string}")
	public void user_checks_for_consent_and_if_required_provided_redirect_url_as_and_x_device_id_as(String bankName,
			String action, String redirectURL, String xDeviceID) {
		try {
